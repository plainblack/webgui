...
my $toVersion = "7.3.20"; # make this match what version you're going to
...
changeBlankMatrixListings($session);
...
#-------------------------------------------------
sub changeBlankMatrixListings {
	my $session = shift;
	print "\tChange all empty product names in Matrix Listings to be a default value.\n" unless ($quiet);
    $session->db->write("update Matrix_listing set productName='Undefined' where productName=''");
}
