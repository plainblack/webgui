...
unless (grep /^IndexedSearch$/, @newWobjects){ push(@newWobjects,"IndexedSearch"); }
unless (grep /^WSClient$/, @newWobjects){ push(@newWobjects,"WSClient"); }
...
if ( grep /^style$/i, @tables ){
	$sth = WebGUI::SQL->read("select * from style");
	...
	print "\t\tstyle table does not exist.\n" unless ($quiet);
}


#--------------------------------------------
print "\tMigrating extra columns to page templates.\n" unless ($quiet);
if ( grep /^ExtraColumn$/i, @tables ){
	...
	print "\t\tExtraColumn table does not exist.\n" unless ($quiet);
}
...
while (my $template = $sth->hashRef) {
	...
	unless ($template->{template} =~ m/draggable\.js/) {
		$template->{template} = '
...
if( grep /^identifier$/i, WebGUI::SQL->buildArray("SHOW COLUMNS FROM SQLReport") ){
	my %dblink;
	...
	print "\t\tDSN mods already done.\n" unless ($quiet);
}

if( grep /^template$/i, WebGUI::SQL->buildArray("SHOW COLUMNS FROM SQLReport") ){
	...
	print "\t\ttemplate mods already done.\n" unless ($quiet);
}

#--------------------------------------------
print "\tConverting items into articles.\n" unless ($quiet);
if ( grep /^Item$/i, @tables ){
...
if( !(grep /^userDefined5$/, (WebGUI::SQL->buildArray("SHOW COLUMNS FROM USS_submission")) ) ){
	...
	print "\t\tAlready Sequenced.\n" unless ($quiet);
}

#--------------------------------------------
	print "\tConverting FAQs into USS Submissions.\n" unless ($quiet);
if ( grep /^FAQ$/i, @tables ){
	my %tempMatch = ();
...
if ( grep /^LinkList$/i, @tables ){
	...
	print "\t\tLinkList table does not exist.\n" unless ($quiet);
}
...
my @unlinks = ( "../../lib/WebGUI/Wobject/Item.pm", \
		"../../lib/WebGUI/Wobject/LinkList.pm", \
		"../../lib/WebGUI/Wobject/FAQ.pm", \
		"../../lib/WebGUI/Wobject/ExtraColumn.pm", \
		"../../lib/WebGUI/Authentication.pm", \
		"../../lib/WebGUI/Operation/Account.pm", \
		"../../lib/WebGUI/Macro/m_currentMenuHorizontal.pm", \
		"../../lib/WebGUI/Macro/M_currentMenuVertical.pm", \
		"../../lib/WebGUI/Macro/s_specificMenuHorizontal.pm", \
		"../../lib/WebGUI/Macro/S_specificMenuVertical.pm", \
		"../../lib/WebGUI/Macro/t_topMenuHorizontal.pm", \
		"../../lib/WebGUI/Macro/T_topMenuVertical.pm", \
		"../../lib/WebGUI/Macro/p_previousMenuHorizontal.pm", \
		"../../lib/WebGUI/Macro/P_previousMenuVertical.pm", \
		"../../lib/WebGUI/Macro/C_crumbTrail.pm", \
		"../../lib/WebGUI/Macro/FlexMenu.pm", \
		"../../lib/WebGUI/Macro/PreviousDropMenu.pm", \
		"../../lib/WebGUI/Macro/Synopsis.pm", \
		"../../lib/WebGUI/Macro/rootmenuHorizontal.pm", \
		"../../lib/WebGUI/Macro/RootTab.pm", \
		"../../lib/WebGUI/Macro/SpecificDropMenu.pm", \
		"../../lib/WebGUI/Macro/TopDropMenu.pm", \
		"../../lib/WebGUI/Macro/Question_search.pm", \
		"../../lib/WebGUI/Operation/Search.pm" );
unlink(@unlinks);
my @rmtrees = ( "../../lib/WebGUI/Authentication", \
		"../../www/extras/toolbar/default" );
rmtree(@rmtrees);

#--------------------------------------------
print "\tMigrating wobject privileges.\n" unless ($quiet);
if( !(grep /^wobjectPrivileges$/i, WebGUI::SQL->buildArray("SHOW COLUMNS FROM page")) ){
...
#--------------------------------------------
if( !(grep /^gotoQuestion$/i, WebGUI::SQL->buildArray("SHOW COLUMNS FROM Survey_question")) ){
...
#	use Data::Dumper; print "\n\n". Dumper($properties);
	my $navId = (getNextId("navigationId") || WebGUI::SQL->quickArray("SELECT MAX(navigationId) FROM Navigation") + 1);
