    ...
    for my $table (@tables) {
        ...
        while (my $row = $sth->fetchrow_hashref) {
            if ($row->{Type} =~ /TEXT/i || $row->{Type} =~ /VARCHAR/i) {
                ...
                push @stmts, sprintf("ALTER TABLE %s CHANGE %s %s %s CHARACTER SET utf8 DEFAULT %s", $dbh->quote_identifier($table), $dbh->quote_identifier($row->{Field}), $dbh->quote_identifier($row->{Field}), $row->{Type}, $dbh->quote($row->{Default}));
            }
        }
        $sth->finish;
