...
function getPopUpMenuObj(e) {
	...
	if (targetElm.tagName=='TD') {
	   ...
	   var trid = tr.id;
       //Don't pop the menu if the user right clicks the column headers or footers
	   if(trid == "") return null;
	   var trspl = trid.split(/::/);
	   //Not on a valid table row
	   if(trspl.length == 0) return null;
	   var taskId = trspl[1];	   
	   
	   cursorMenu1.addMenuItem(new menuItem("Edit Task", "","code:buildMenuUrl('edit','"+taskId+"');"));
	   cursorMenu1.addMenuItem(new menuItem("Insert Task Above", "","code:buildMenuUrl('insertAbove','"+taskId+"');"));
	   cursorMenu1.addMenuItem(new menuItem("Insert Task Below", "","code:buildMenuUrl('insertBelow','"+taskId+"');"));
	   cursorMenu1.addMenuItem(new menuItem("-"));
       cursorMenu1.addMenuItem(new menuItem("Delete Task", "","code:buildMenuUrl('delete','"+taskId+"');"));
	   return cursorMenu1;
    }
  	 return null;
}
...
function getColID(el) 
{
	if (el.tagName=='TH')
		...
		return table.getElementsByTagName('TBODY')[0].getElementsByTagName('TR')[0].getElementsByTagName('TD')[realIndex].id;
}
